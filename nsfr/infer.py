...
class InferModule(nn.Module):
    ...
    def forward(self, x):
        ...
        Perform forward-chaining inference for a specified number of steps.

        Args:
            x: Initial valuation tensor [batch_size, num_atoms] with truth values
               This comes from the FactsConverter

        Returns: 
            R: Final valuation tensor after applying rules
        """
        R = x
    ...
    def r(self, x):
        """
        Apply all clauses to current valuation to derive new facts.

        This is where the encoded rules (I tensor) are applied to the current valuation
        to derive new facts
        """
...
class ClauseFunction(nn.Module):
    ...
    Apply a singel logical clause to derive new facts
    """
